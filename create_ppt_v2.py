...
(v2 — Dark Navy + Gold color scheme)
"""
...
# ── Constants (v2 color scheme) ───────────────────────────────────────
NAVY   = RGBColor(0x1B, 0x2A, 0x44)      # Primary dark navy
GOLD   = RGBColor(0xC8, 0xA8, 0x4B)      # Accent gold
WHITE  = RGBColor(0xFF, 0xFF, 0xFF)
DARK   = RGBColor(0x22, 0x22, 0x22)      # Text on light
LIGHT_BG = RGBColor(0xF5, 0xF3, 0xEE)   # Slide background
LIGHT_NAVY = RGBColor(0x1B, 0x2A, 0x44)  # Same as primary navy (was #2D3E5A)
GOLD_DARK = RGBColor(0xC8, 0xA8, 0x4B)   # Unified to gold (was #A0863C)
LIGHT_BLUE = RGBColor(0xDC, 0xE8, 0xF8)  # Light navy for insight boxes
SECONDARY_TEXT = RGBColor(0x66, 0x66, 0x66)  # Secondary text on light
...
add_textbox(s, Inches(1.0), Inches(4.8), Inches(11.0), Inches(0.5),
            ...
            font_size=15, color=SECONDARY_TEXT, alignment=PP_ALIGN.CENTER)
...
for i, (cat_name, cat_color, techniques) in enumerate(categories):
    ...
    for j, tech in enumerate(techniques):
        ...
        row_color = WHITE if j % 2 == 0 else RGBColor(0xEC, 0xF0, 0xF8)
...
add_textbox(s, Inches(0.9), Inches(6.3), Inches(11.0), Inches(0.4),
            ...
            font_size=12, color=SECONDARY_TEXT, alignment=PP_ALIGN.LEFT)
...
add_textbox(s, Inches(0.9), Inches(6.3), Inches(11.0), Inches(0.4),
            ...
            font_size=11, color=SECONDARY_TEXT)
...
# Highlight box
box = add_rounded_rect(s, Inches(2.0), Inches(5.5), Inches(9.3), Inches(0.9), LIGHT_BLUE, NAVY)
set_shape_text(box,
...
for j, (label, *cells) in enumerate(rows):
    ...
    bg = WHITE if j % 2 == 0 else RGBColor(0xEC, 0xF0, 0xF8)
    # Label col
